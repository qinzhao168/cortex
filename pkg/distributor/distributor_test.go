	...
	for name, tc := range map[string]struct {
		...
		for _, shardByAllLabels := range []bool{true, false} {
			t.Run(fmt.Sprintf("[%s](shardByAllLabels=%v)", name, shardByAllLabels), func(t *testing.T) {
				...
				d := prepare(t, tc.numIngesters, tc.happyIngesters, 0, shardByAllLabels, limits, nil)
				defer d.Stop()
	...
	for i, tc := range []struct {
		...
		for _, shardByAllLabels := range []bool{true, false} {
			t.Run(fmt.Sprintf("[%d](shardByAllLabels=%v)", i, shardByAllLabels), func(t *testing.T) {
				...
				d := prepare(t, 1, 1, 0, shardByAllLabels, &limits, nil)
...
func TestDistributor_PushQuery(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			d := prepare(t, tc.numIngesters, tc.happyIngesters, 0, tc.shardByAllLabels, nil, nil)
			defer d.Stop()
...
func TestDistributorValidateSeriesLabelRemoval(t *testing.T) {
	...
	for _, tc := range cases {
		...
		d := prepare(t, 1, 1, 0, true, &limits, nil)
...
func TestSlowQueries(t *testing.T) {
	...
	for _, shardByAllLabels := range []bool{true, false} {
		for happy := 0; happy <= nIngesters; happy++ {
			...
			d := prepare(t, nIngesters, happy, 100*time.Millisecond, shardByAllLabels, nil, nil)
			defer d.Stop()
...
func TestDistributor_MetricsForLabelMatchers(t *testing.T) {
	...
	// Create distributor
	d := prepare(t, 3, 3, time.Duration(0), true, nil, nil)
	defer d.Stop()
...
func TestDistributorValidation(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			d := prepare(t, 3, 3, 0, true, &limits, nil)
			defer d.Stop()
